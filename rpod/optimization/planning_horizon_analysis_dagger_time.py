        ...
        if not out['cvx_problem']:
            ...
            try:
                ...
                for i in np.arange(rpod.n_time_rpod):        
                    ...
                    #env_myocvxMPC.load_prediction(myocvx_traj, myocvxMPC_traj)
...
if __name__ == '__main__':
    ...
    for n_steps in mpc_steps_analysis:
        ...
        for i in tqdm(range(len(uniform_idx)), total=len(uniform_idx)):
            ...
            if i%200 == 0:
                np.savez_compressed(root_folder + '/optimization/saved_files/closed_loop/ws_analysis_time_myoDAGGER' + str(other_args['mpc_steps']) + '_' + str(i),
                            J_cvx = J_cvx,
                            J_cvxMPC = J_cvxMPC,
        ...
        np.savez_compressed(root_folder + '/optimization/saved_files/closed_loop/ws_analysis_time_myoDAGGER' + str(other_args['mpc_steps']),
                        J_cvx = J_cvx,
                        J_cvxMPC = J_cvxMPC,
                        J_myocvxMPC = J_myocvxMPC,
                        J_artMPC = J_artMPC,
